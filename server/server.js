...
async function assertPublicUrl(raw) {
  ...
  if (net.isIP(host)) { if (isPrivateIp(host)) throw new Error('private IP not allowed'); return; }
  let addrs;
...
app.post('/feedback', async (req, res) => {
  ...
  if (rateLimited('fb:' + ip)) return res.status(429).json({ error: 'too many requests' });
...
app.post('/capture', async (req, res) => {
  ...
  const ip = (req.headers['x-forwarded-for'] || '').split(',')[0].trim() || req.ip || 'unknown';
  if (rateLimited(ip)) return res.status(429).json({ error: 'too many requests, wait a minute' });
  try { await assertPublicUrl(url); } catch (e) { return res.status(400).json({ error: e.message }); }
  if (activeCaptures >= MAX_CONCURRENT) return res.status(503).json({ error: 'server is busy rendering, retry in a few seconds' });
  activeCaptures++;
...
async function gotoResilient(page, url) {
  ...
  for (let i = 0; i < 2; i++) {
    try {
      ...
      const cur = page.url();
      if (cur.startsWith('chrome-error://')) throw new Error('page failed to load (network error)');
      if (resp && resp.status() >= 400) throw new Error('site returned HTTP ' + resp.status());
  ...
  throw new Error('failed to open ' + url + ' — ' + (lastErr && lastErr.message || lastErr));
}
